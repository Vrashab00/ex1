...
import { LLMAuditOutput, LLMInstanceVerdict, llmAuditOutputSchema } from '../schemas/auditSchemas.js';

export class MockRuleEngineProvider implements AIProvider {
  readonly name = 'FinOps Heuristic Engine (Built-in)';

  async auditFleet(instances: InstanceTelemetry[], options?: AuditAuditOptions): Promise<LLMAuditOutput> {
    const verdicts: LLMInstanceVerdict[] = [];
    let totalSavings = 0;
    ...
    for (const inst of instances) {
      ...
      let recommendedAction: 'terminate' | 'keep' | 'downscale' | 'investigate';
      let estimatedMonthlySavings: number = 0;
    ...
    return llmAuditOutputSchema.parse(rawOutput);
  }
}
